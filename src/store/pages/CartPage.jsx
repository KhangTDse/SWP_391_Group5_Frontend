  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        ...
        <Link
          to="/shop"
          className="mt-4 bg-amber-600 text-white px-6 py-3 rounded-lg hover:bg-amber-700 font-bold transition"
        >
          Mua sắm ngay
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {cartItems.map((item) => (
              <div
                key={item.id}
                className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row gap-4 items-center"
              >
                <img
                  src={item.image}
                  alt={item.name}
                ...
                <div className="flex flex-col items-center sm:items-end gap-2">
                  <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                    ...
                    <span className="px-3 py-1 font-medium min-w-[30px] text-center">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => handleQuantityChange(item.id, 1)}
          ...
          <div className="h-fit bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-4">
            ...
            <button
              ...
              className="w-full bg-black text-white py-4 rounded-lg font-bold hover:bg-amber-600 transition shadow-lg"
            >
              THANH TOÁN
            </button>
          </div>
